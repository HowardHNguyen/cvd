...
with col1:
    ...
    totchol = st.slider('Total Cholesterol:', min_value=0, max_value=600, value=200)
    sysbp = st.slider('Systolic Blood Pressure:', min_value=0, max_value=300, value=120)
    diabp = st.slider('Diastolic Blood Pressure:', min_value=0, max_value=200, value=80)
    bmi = st.slider('BMI:', min_value=0.0, max_value=100.0, value=25.0)
    cursmoke = st.selectbox('Current Smoker:', [0, 1])
    glucose = st.slider('Glucose:', min_value=0, max_value=500, value=100)
    diabetes = st.selectbox('Diabetes:', [0, 1])
    heartrate = st.slider('Heart Rate:', min_value=0, max_value=300, value=70)
    cigpday = st.slider('Cigarettes Per Day:', min_value=0, max_value=100, value=0)
